...
router.get('/', function (req, res) {
  let content = render(LOAN_FORM_TEMPLATE, {apr: RATE});
...
function createLoanOffer(data) {
  ...
  data.RATE = RATE;
  data.payment = Number(calculatePayment(data.amount, data.duration, data.RATE)).toFixed(2);
...
function calculatePayment(amount, duration, rate) {
  ...
  let loanMonths = duration * 12;
  return amount * (monthlyRate / (1 - ((1 + monthlyRate) ** -loanMonths)));
}
